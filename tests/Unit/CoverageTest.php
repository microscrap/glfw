 ...
 * committed 0.5.0 stub snapshot — so the guard runs everywhere.
 */
function extensionMethods(string $extensionClass): array
{
    if (extension_loaded('glfw') && class_exists($extensionClass)) {
        return get_class_methods($extensionClass);
    }

    static $snapshot = null;
    if (is_null($snapshot)) {
        $snapshot = require __DIR__ . '/../Support/extension-methods-0.5.0.php';
    }
